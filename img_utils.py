...
def drawAxesWithPose(img, ar_corner, ar_idx, dict_world_loc):
    ...
    axis = np.float32([[0,0,0], [25,0,0], [0, 25, 0], [0, 0, 0-25]])

    # Convert the points from marker coordinate system to image coordinate system 

    # Find the rotation and translation vectors.
    ret, rvecs, tvecs = cv2.solvePnP(dict_world_loc[ar_idx], ar_corner, mtx, dist)
    ...
    cv2.putText(img, str(ar_idx), tuple(img_pts[0].ravel().astype(int)), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (255, 0, 0))
