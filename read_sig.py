...
def get_id(binary):
    ...
    signature = []
    for i in range(4):
        with open('marker/sample2/triangle_list{}.txt'.format(i)) as file :
            for line in file :
                ...
                # cv2.circle(img, (int(c[0]), int(c[1])), 4 , (0, 255, 0), cv2.FILLED)
                if (binary[int(c[0]), int(c[1])] >= 128):
                ...
                # cv2.polylines(img, [x], True, (0, 0, 0), 2)

    return signature

# im, sig = get_id(binary)
